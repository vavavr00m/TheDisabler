...
for /f usebackq^tokens^=1-3*delims^=^" %%i in =;(
    `type "%~f0" ^| findstr /b \^"`
		);= do if exist "%%A:%%~k\%commons%\%specific%" ( echo\ && echo Exists - "%%A:%%~k\%commons%\%specific%"
